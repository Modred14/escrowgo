...
import {
  Wallet, ArrowDownToLine, ArrowUpFromLine, Receipt, CreditCard,
  Sparkles, CheckCircle2, XCircle, AlertTriangle, Search, ArrowRight,
} from "lucide-react";
...
/* Real wallet data — swap for API data */
function useWalletData() {
  const [data] = useState({
    ...
    sales: [
      { id: "TX-5510", buyer: "John Micheal", product: "Samsung S21 Ultra", amount: 250000, status: "Delivered" },
      { id: "TX-5509", buyer: "Kunle Ogundiran", product: "Hoodie", amount: 50000, status: "Undelivered" },
      { id: "TX-5508", buyer: "Joshua Kimmich", product: "Samsung S23 Ultra", amount: 450000, status: "Delivered" },
      { id: "TX-5507", buyer: "Kelvin Henry", product: "iPhone 14 Pro", amount: 750000, status: "Undelivered" },
      { id: "TX-5506", buyer: "Faiq Modred", product: "iPhone 14 Pro", amount: 750000, status: "In Dispute" },
    ],
  });
  return data;
}

function MiniStat({ icon: Icon, label, value, prefix = "", tint, delay, mounted }) {
  ...
  return (
    ...
    >
      ...
      <p className="mt-1 font-serif text-[24px] font-semibold tracking-tight" style={{ color: tint.fg }}>
        {prefix}{Number(count).toLocaleString("en-NG")}
      </p>
    </div>
  );
}
...
export default function WalletTransactions() {
  ...
  return (
    <div>
      {/* Header */}
      <div className="flex items-start justify-between opacity-0 animate-riseIn" style={{ animationDelay: "60ms" }}>
        <div>
        ...
        <div className="flex flex-col justify-between gap-6 sm:flex-row sm:items-center">
          <div>
            <div className="flex items-center gap-2">
              <div className="flex h-6 w-6 items-center justify-center rounded-full" style={{ backgroundColor: "rgba(22,19,13,0.1)" }}>
                <Sparkles size={12} style={{ color: C.goldDeep }} />
              </div>
              <span className="text-[11px] font-semibold uppercase tracking-[0.14em]" style={{ color: C.goldDeep }}>
                EscrowGo Wallet Balance
              </span>
            </div>
            <p className="mt-3 text-[13px]" style={{ color: "rgba(22,19,13,0.55)" }}>Available balance</p>
            <p className="mt-1 font-serif text-[38px] font-semibold tracking-tight" style={{ color: C.ink }}>
          ...
          <div className="relative hidden h-[140px] w-[160px] shrink-0 sm:block">
            <div
              ...
              style={{ backgroundColor: C.ink, boxShadow: "0 20px 40px -16px rgba(22,19,13,0.4)" }}
            />
            <div
              ...
              style={{ background: `linear-gradient(135deg, ${C.inkSoft}, ${C.ink})`, boxShadow: "0 16px 30px -14px rgba(22,19,13,0.35)" }}
            />
            <div
              className="absolute bottom-1 right-6 flex h-[74px] w-[120px] items-center justify-center rounded-xl"
              style={{ background: `linear-gradient(135deg, ${C.goldSoft}, ${C.gold})`, boxShadow: "0 14px 26px -10px rgba(198,156,63,0.55)" }}
            >
            ...
            <div
              ...
              style={{ backgroundColor: C.cream, boxShadow: "0 6px 14px -4px rgba(22,19,13,0.3)" }}
            >
      ...
      <div className="mt-6 grid grid-cols-1 gap-4 sm:grid-cols-3">
        <MiniStat icon={ArrowDownToLine} label="Money in" value={moneyIn} prefix="₦" tint={{ bg: C.greenSoft, fg: C.green }} mounted={mounted} delay={200} />
        <MiniStat icon={ArrowUpFromLine} label="Total withdrawn" value={totalWithdrawn} prefix="₦" tint={{ bg: C.redSoft, fg: C.red }} mounted={mounted} delay={250} />
        <MiniStat icon={Receipt} label="Total transactions" value={totalTransactions} tint={{ bg: "#FBF0DE", fg: C.goldDeep }} mounted={mounted} delay={300} />
      </div>
      ...
      >
        <div className="flex flex-col gap-3 border-b p-5 sm:flex-row sm:items-center sm:justify-between" style={{ borderColor: C.line }}>
          <p className="font-serif text-[17px] font-semibold" style={{ color: C.ink }}>Sales history</p>
          <div className="flex items-center gap-3">
            <div className="relative">
              <Search size={14} className="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2" style={{ color: C.textMuted }} />
              <input
                value={query}
        ...
        <div className="overflow-x-auto">
          <table className="w-full min-w-[640px] border-collapse">
            <thead>
              <tr>
                {["Buyer name", "Product", "Total amount", "Delivery status"].map((h) => (
                  <th
                    key={h}
            ...
            <tbody>
              {filteredSales.map((row, i) => (
                ...
                >
                  <td className="px-5 py-3.5 text-[13.5px] font-medium" style={{ color: C.ink }}>{row.buyer}</td>
                  <td className="px-5 py-3.5 text-[13.5px]" style={{ color: C.inkFaint }}>{row.product}</td>
                  <td className="px-5 py-3.5 text-[13.5px]" style={{ color: C.textMuted }}>{formatNaira(row.amount)}</td>
                  <td className="px-5 py-3.5"><StatusBadge status={row.status} /></td>
                </tr>
              ))}
              {filteredSales.length === 0 && (
                <tr>
                  <td colSpan={4} className="px-5 py-8 text-center text-[13px]" style={{ color: C.textMuted }}>
