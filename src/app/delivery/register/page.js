    ...
    <main className="flex min-h-screen items-center justify-center bg-paper px-4 py-12">
      <div className="w-full max-w-sm">
        <div className="mb-8 flex flex-col items-center">
          <SealMark size={44} />
          <h1 className="mt-4 font-display text-2xl font-semibold text-ink">Become an escrowgo courier</h1>
          <p className="mt-1 text-sm text-ink/55">Accept deliveries near you and get paid per drop-off.</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4 rounded-2xl border border-ink/10 bg-white p-6 shadow-card">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="mb-1.5 block text-sm font-medium text-ink/70">Base location</label>
              <select
                value={form.location}
            ...
            <div>
              <label className="mb-1.5 block text-sm font-medium text-ink/70">Vehicle</label>
              <select
