...
function LocationPicker({ label, value, onChange }) {
  ...
  // Fetch states for the selected country, from GeoNames (so codes line up later)
  useEffect(() => {
    if (!countryCode) {
      setStates([]);
      return;
  ...
  // Fetch cities/towns scoped to the selected state via adminCode1 (exact code match)
  useEffect(() => {
